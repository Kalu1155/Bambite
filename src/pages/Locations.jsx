...
import locimg from "../assets/images/food_3.jpeg"
              ...
              {dummyRestaurants.map((rest) => (
                <Marker key={rest.id} position={[rest.lat, rest.lng]}>
                  <Popup>
                    ...
                    📞 {rest.phone} <br />
                    ⭐ {rest.rating} <br />
                    <button className="pri-btn" onClick={() => (window.location.href = `/reservation?restId=${rest.id}`)}>
                      Reserve Table
                    </button>
                  </Popup>
                </Marker>
